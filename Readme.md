...
This repository contains a simple Python script for importing conversations from openAI's chatGPT export format. It relies on the x-callback mechanism in the Bear app to create notes from the conversations. 

The import is done in two steps. First, the Python script will generate a `bear://` URL for each conversation and open
it in the Bear app. The standard browser is used to handle the callback from the Bear app. During the import the script
will open a browser window for each conversation. The browser will call the Node.js server with the note information.
The server will append the note information to the `bear_import_log.jsonl` file. The Python script will then write an
